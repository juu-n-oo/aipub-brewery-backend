...
import io.ten1010.imagekitbackend.common.exception.DuplicateResourceException;
...
public class DockerfileService {
    ...
    public List<DockerfileResponse> listAll() {
        return mapper.toResponseList(repository.findAllByOrderByCreatedAtDesc());
    }

    /** 관리자 전체 조회(username 필터): 해당 사용자 소유 Dockerfile 만 생성 일시 최신순으로 반환한다. */
    public List<DockerfileResponse> listAll(String usernameFilter) {
        return mapper.toResponseList(repository.findByUsernameOrderByCreatedAtDesc(usernameFilter));
    }
