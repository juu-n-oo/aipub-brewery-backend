...
public class KubernetesConfiguration {
    ...
    public ApiClient apiClient(K8sProperties k8sProperties) throws IOException {
        Objects.requireNonNull(k8sProperties.getKubeConfig(), "imagekit.kubernetes.kube-config must be configured");
        Objects.requireNonNull(k8sProperties.getKubeConfig().getMode(), "imagekit.kubernetes.kube-config.mode must be configured");
        Objects.requireNonNull(k8sProperties.getVerifySsl(), "imagekit.kubernetes.verify-ssl must be configured");
        ...
        return switch (kubeConfigProperty.getMode()) {
            ...
            case FILE -> {
                Objects.requireNonNull(kubeConfigProperty.getKubeConfigPath(),
                        "imagekit.kubernetes.kube-config.kube-config-path must be configured when mode is FILE");
